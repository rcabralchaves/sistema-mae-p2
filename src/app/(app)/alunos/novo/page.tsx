...
export default function NovoAlunoPage() {
  ...
  const [form, setForm] = useState({ nome: '', dataNascimento: '', escola: '', serie: '', turma: '', turno: '', responsavel: '', telefone: '', email: '', convenio: '', observacoes: '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/alunos', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    if (res.ok) { const a = await res.json(); router.push(`/alunos/${a.id}`) }
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, fontSize: 13, color: '#6b7280' }}>
        <Link href="/alunos" style={{ color: '#6b7280' }}>Alunos</Link>
        <span>/</span>
        <span style={{ color: '#111827', fontWeight: 500 }}>Novo aluno</span>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Section title="Dados escolares">
          <Grid cols={2}>
            ...
            <Field label="Turma"><input value={form.turma} onChange={e => set('turma', e.target.value)} style={iStyle} /></Field>
            <Field label="Turno">
              <select value={form.turno} onChange={e => set('turno', e.target.value)} style={iStyle}>
        ...
        <Section title="Observações">
          <textarea rows={3} value={form.observacoes} onChange={e => set('observacoes', e.target.value)} style={{ ...iStyle, resize: 'vertical' }} />
        </Section>
